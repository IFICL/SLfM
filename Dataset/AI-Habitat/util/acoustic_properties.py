        ...
        if self.randomize_label:
            ...
            for category, acoustic_mats in CATEGORY_TO_ACOUSTIC_MATERIAL_MAPPING.items():
                if len(acoustic_mats) == 1:
                    random_acoustic_material_label_mapping[acoustic_mats[0]].append(
                        category)
                else:
                    random_acoustic_material_label_mapping[np.random.choice(
                        acoustic_mats)].append(category)
    ...
    def randomize_coeff(self, coefficients):
        ...
        for i in range(len(coefficients)):
            if i % 2 == 0:
                ...
                new_coeff.append(
                    np.clip(coefficients[i] + np.random.normal(0, self.std), 0, 1))
...
if __name__ == '__main__':
    generator = Mp3dMaterialConfigGenerator()
    save_path = 'hm3d_material_config.json'
