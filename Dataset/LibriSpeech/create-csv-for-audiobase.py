...
def create_list_for_video(args, name, video_list):
    ...
    for videos in tqdm(video_list):
        subvideo_list = glob.glob(f'{videos}/*/*')
        subvideo_list.sort()
        for video in subvideo_list:
            path = os.path.join('./data/AI-Habitat/LibriSpeech', video)
...
# Usage: python create-csv-for-audiobase.py --type='LibriSpeech' --data_split='7:2:1'
